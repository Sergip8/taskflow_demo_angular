...
import { SearchTableResponse } from "../../../models/search-table-response";
...
@Component({
    ...
    template: `
   <div>
       <form class="max-w-md mx-auto w-full" (submit)="onSubmit($event)">   
       ...
       @for (item of results; track $index) {
           ...
           <div (click)="onClick(item.id)" class="absolute mt-2 w-full overflow-hidden rounded-md bg-white">
                   <div class="cursor-pointer py-2 px-3 hover:bg-slate-100">
                     <p class="text-sm font-medium text-gray-600">{{item.filenames}}</p>
                   </div>
           </div>
   }
    </form>
...
onClick(selected: string) {
    ...
    @Input() results: SearchTableResponse[] = []
    @Output() selected = new EventEmitter<string>()
    onSubmit(event: any) {
        event.preventDefault();
        this.search.emit(event.target.search.value);
    }
}